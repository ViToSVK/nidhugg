...
DPORDriver::Result DPORDriver::run(){
  ...
  case Configuration::POWER:
    ...
    break;
  case Configuration::INITSC:
    TB = new ZBuilderSC(conf, mod, true); // initial-trace-only-true
    break;
  case Configuration::VC:
    TB = new ZBuilderSC(conf, mod, false); // initial-trace-only-false
    break;
  case Configuration::MM_UNDEF:
    throw std::logic_error("DPORDriver: No memory model is specified.");
    break;
  ...
  if (conf.memory_model == Configuration::VC ||
      conf.memory_model == Configuration::INITSC) {
    run_once(*TB);
